    ...
    def collision_check(self, paths, obstacles):
        ...
        for i in range(len(paths)):
            ...
            for j in range(len(path[0])):
                ...
                for idx , offset in enumerate(self._circle_offsets):
                    circle_locations[idx, 0] = x + offset * cos(yaw)
                    circle_locations[idx, 1] = y + offset * sin(yaw)
                # --------------------------------------------------------------
    ...
    def select_best_path_index(self, paths, collision_check_array, goal_state):
        ...
        for i in range(len(paths)):
            ...
            if collision_check_array[i]:
                ...
                # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
                # --------------------------------------------------------------
                x_end  = paths[i][0][-1] # last point's x value
                y_end  = paths[i][1][-1] # last point's y value
                x_goal = goal_state[0]
                y_goal = goal_state[1]
                score = np.sqrt((x_goal - x_end)**2 + (y_goal - y_end)**2)
                # --------------------------------------------------------------
                ...
                for j in range(len(paths)):
                    if j == i:
                        ...
                        if not collision_check_array[j]:
                            # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
                            # --------------------------------------------------
                            x_val  = paths[i][0][-1] # last point's x value of our valid path 
                            y_val  = paths[i][1][-1] # last point's y value of our valid path 
                            x_col  = paths[j][0][-1] # last point's x value of collision path 
                            y_col  = paths[j][1][-1] # last point's y value of collision path 
                            # if they are near don't pick 
                            dist = np.sqrt((x_col - x_val)**2 + (y_col - y_val)**2)
                            if dist > 0:
                                score  += self._weight * (1.0/dist)
