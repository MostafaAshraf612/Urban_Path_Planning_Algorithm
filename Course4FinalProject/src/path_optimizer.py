...
class PathOptimizer:
    ...
    def thetaf(self, a, b, c, d, s):
        # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
        # ------------------------------------------------------------------
        # Convert everything to numpy arrays (works even if inputs are scalars)
        a = np.array(a)
        b = np.array(b)
        c = np.array(c)
        d = np.array(d)
    ...
    def sample_spiral(self, p):
        ...
        cos_t = np.cos(t_points)
        sin_t = np.sin(t_points)
        #Integrate to get x and y. Use cumtrapz with initial=0 to match lengths.
        x_points = scipy.integrate.cumtrapz(cos_t, s_points, initial=0.0)
        y_points = scipy.integrate.cumtrapz(sin_t, s_points, initial=0.0)
