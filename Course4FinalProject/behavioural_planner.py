...
class BehaviouralPlanner:
    ...
    def transition_state(self, waypoints, ego_state, closed_loop_speed):
        ...
        # the stop sign and transition to the next state.
            ...
            if self._stop_count == STOP_COUNTS:
                ...
                # Nudge the goal position away from curb by shifting y (or x) coordinate slightly
                safe_offset = 1  # meters, tune as needed
                self._goal_index = goal_index
                x_goal ,y_goal ,v_goal = waypoints[goal_index]
                x_goal += safe_offset
    ...
    def get_goal_index(self, waypoints, ego_state, closest_len, closest_index):
        ...
        while wp_index < len(waypoints) - 1:
            x_prev = waypoints[wp_index][0]
            y_prev = waypoints[wp_index][1]
            x_next = waypoints[wp_index+1][0]
            y_next = waypoints[wp_index+1][1]
            arc_length += np.sqrt((x_next - x_prev)**2 + (y_next - y_prev)**2)
            if arc_length >=self._lookahead:
                wp_index +=1
                return wp_index
            wp_index += 1 # increment index
        # Fallback return to prevent None
    ...
    def check_for_stop_signs(self, waypoints, closest_index, goal_index):
        ...
        """
        if goal_index is None or closest_index is None or goal_index <= closest_index:
...
def get_closest_index(waypoints, ego_state):
    ...
    for i in range(len(waypoints)):
        x_way = waypoints[i][0]
        y_way = waypoints[i][1]
        dist = np.sqrt((x_way - x_ego)**2 + (y_way - y_ego)**2)
