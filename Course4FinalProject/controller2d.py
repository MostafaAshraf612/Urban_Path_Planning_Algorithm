...
class Controller2D(object):
    def __init__(self, waypoints):
        ...
        self._default_lookahead_distance = 2.0
        self._lookahead_distance    = self._default_lookahead_distance
        self._current_x             = 0
        self._current_y             = 0
        self._current_yaw           = 0
        ...
        self._pi                    = np.pi
        self._2pi                   = 2.0 * np.pi
        self._steer_smooth_factor   = 0.3  # smoothing gain for steering command

    def update_values(self, x, y, yaw, speed, timestamp, frame, stopped=False):
        self._current_x         = x
        self._current_y         = y
        ...
        # Temporarily reduce lookahead distance if recently stopped for smoother steering
        if stopped and self._start_control_loop:
            self._lookahead_distance = 1.0  # reduce lookahead when starting from stop
        else:
            self._lookahead_distance = self._default_lookahead_distance
    ...
    def set_steer(self, input_steer_in_rad):
        ...
        # Apply simple smoothing filter to steering to reduce abrupt changes
        self._set_steer = (self._steer_smooth_factor * steer +
                           (1 - self._steer_smooth_factor) * self._set_steer)
    ...
    def update_controls(self):
        ...
        self.vars.create_var('kp_heading', 6.00)  # Reduced from 8.0 to soften steering
        self.vars.create_var('k_speed_crosstrack', 0.1)  # Added small velocity scaling
        self.vars.create_var('cross_track_deadband', 0.05)  # Increased deadband from 0.01 to 0.05
